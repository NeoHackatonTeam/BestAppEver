...
struct OnboardingTutorialView: View {
    ...
    private func getOrbitImageName(orbit: AppDataModel.Orbit) -> String? {
        guard let session = data.objectCaptureSession else { return nil }
        let orbitCompleted = session.userCompletedScanPass
        let orbitCompleteImage = orbit <= data.orbit ? orbit.imageSelected : orbit.image
        let orbitNotCompleteImage = orbit < data.orbit ? orbit.imageSelected : orbit.image

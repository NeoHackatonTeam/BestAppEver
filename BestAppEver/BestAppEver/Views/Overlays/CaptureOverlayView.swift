    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            if shouldShowTutorial, let url = Bundle.main.url(
                forResource: data.orbit.feedbackVideoName(
                    for: UIDevice.current.userInterfaceIdiom,
                    isObjectFlippable: data.isObjectFlippable),
                withExtension: "mp4") {
            ...
            HStack(alignment: .bottom, spacing: 0) {
                ...
                if !capturingStarted {
                    CaptureButton(session: session, isObjectFlipped: data.isObjectFlipped, hasDetectionFailed: $hasDetectionFailed)
                        .layoutPriority(1)
                }
        ...
        .background {
            if !shouldShowTutorial && data.messageList.activeMessage != nil {
                VStack {
                    Rectangle()
                        .frame(height: 130)
                        .hidden()

                    FeedbackView(messageList: data.messageList)
                        .layoutPriority(1)
                }
                .rotationEffect(rotationAngle)
            }
